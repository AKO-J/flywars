...
└──────────┘ └──────────┘ └─────────────┘
   ...
   ┌──────────────────────────────────┐
   │          server/server.py        │  ← 独立进程
   │    (asyncio TCP · 房间 · 积分)    │
   └──────────┬───────────────────────┘
              │ 依赖
              ▼
   ┌──────────────────────────────────┐
   │  systems/protocol.py (协议)      │
   │  systems/frame_io.py (帧读写)    │
   │  systems/event_bus.py (事件总线) │
   │  systems/logger.py (日志)        │
   └──────────────────────────────────┘
```

核心数据流: `main.py → Game(core) → 各子系统(systems) + 实体(entities) + 精灵(sprites)`。
模块间通过 **EventBus** 解耦通信，网络模块通过 **NetworkClient** 与 **server/** 交互。
